...
public class OrdersDao {
	...
	public int addOrders(Orders orders, Connection conn){
		String sql = "INSERT INTO orders(orders_id,orders_no,goods_id,goods_name,goods_price,orders_date,user_id,real_name,user_address,user_phone,orders_flag) values(null,?,?,?,?,?,?,?,?,?,?)";
		Object[] params = new Object[] {
			orders.getOrders_no(),
			orders.getGoods_id(),
			orders.getGoods_name(),
			orders.getGoods_price(),
	...
	public List<Orders>  listOrderss(Orders orders, Connection conn){
		...
		if (orders.getSell_id()!=0) {
			sBuilder.append(" and o.goods_id in (select goods_id from goods g where g.user_id=" + orders.getSell_id() +") ");
		}
		if (orders.getOrders_flag()!=0) {
